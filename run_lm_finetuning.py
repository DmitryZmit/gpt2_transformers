...
def train(args, train_dataset, model, tokenizer):
    """ Train the model """
    if (args.local_rank!=-1) and (torch.distributed.get_rank() not in [-1, 0]):
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if (args.local_rank!=-1) and (torch.distributed.get_rank() not in [-1, 0]) and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    # Log metrics
                    if (
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    if not os.path.exists(eval_output_dir) and torch.distributed.get_rank() in [-1, 0]:
...
def main():
    ...
    if args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        if not os.path.exists(args.output_dir) and torch.distributed.get_rank()in [-1, 0]:
    ...
    # Evaluation
    results = {}
    if args.do_eval and torch.distributed.get_rank() in [-1, 0]:
